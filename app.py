# app.py - SQLite версия с московским временем
...
def login():
    if request.method == 'POST':
        ...
        if user and check_password_hash(user['password_hash'], password):
            # Обновляем время последнего входа (московское)
...
def dashboard():
    ...
    cur.execute('SELECT COUNT(*) FROM users WHERE role = "class_teacher"')
    class_teacher_count = cur.fetchone()[0]
    
    cur.execute('SELECT COUNT(*) FROM users WHERE role = "head_teacher"')
    head_teacher_count = cur.fetchone()[0]
    ...
    return render_template('dashboard.html',
                         ...
                         head_teacher_count=head_teacher_count,
...
def add_grade():
    ...
    if existing:
        ...
        cur.execute('''
            ...
            WHERE id = ?
        ''', (grade, comment, moscow_now.isoformat(), existing[0]))
        action = 'UPDATE'
        old_value = json.dumps({'grade': old_grade})
        new_value = json.dumps({'grade': grade})
    else:
        cur.execute('''
            INSERT INTO grades (student_id, subject_id, grade, quarter, comment, teacher_id, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', (student_id, subject_id, grade, quarter, comment, session['user_id'], moscow_now.isoformat()))
        action = 'INSERT'
        old_value = None
        new_value = json.dumps({'grade': grade})
...
def mark_attendance():
    ...
    if existing:
        cur.execute('''
            ...
            WHERE id = ?
        ''', (status, reason, moscow_now.isoformat(), existing[0]))
        action = 'UPDATE'
    else:
        cur.execute('''
            INSERT INTO attendance (student_id, subject_id, lesson_date, status, reason, created_at)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (student_id, subject_id, lesson_date, status, reason, moscow_now.isoformat()))
        action = 'INSERT'
...
def generate_report():
    ...
    # Получаем учащихся класса с их средними баллами (ИСПРАВЛЕННЫЙ ЗАПРОС)
    cur.execute('''
        SELECT 
            s.id, s.last_name, s.first_name, s.middle_name,
            COALESCE(AVG(CAST(g.grade AS REAL)), 0) as average_grade,
    ...
    report = {
        ...
        'quarter': quarter,
        'generated_at': moscow_now.strftime('%d.%m.%Y %H:%M'),
        'students': students
    }
